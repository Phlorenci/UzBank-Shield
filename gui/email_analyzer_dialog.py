"""
Email phishing analysis dialog for the desktop GUI.
"""
...
from core.email_analyzer import analyze_email, assess_email_risk
...
class EmailAnalysisWorker(QObject):
    """
    Runs analyze_email() on a background thread, since it may call
    URLAnalyzer (network I/O) for each URL found in the email.
    """
    ...
    def run(self):
        try:
            result = analyze_email(self.text)
            self.finished.emit(result)
        except Exception as error:
            self.failed.emit(str(error))


class EmailAnalyzerDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.email_input.setMinimumHeight(140)
    ...
    def _retranslate_ui(self):
        self.setWindowTitle(self._t("email_dialog_title"))
        self.email_input.setPlaceholderText(self._t("email_dialog_placeholder"))
        self.analyze_button.setText(self._t("email_dialog_analyze"))
        self.close_button.setText(self._t("email_dialog_close"))
    ...
    def _on_analyze_clicked(self):
        ...
        if not text:
            self.status_label.setText(self._t("email_dialog_empty"))
            return
        ...
        self.thread = QThread()
        self.worker = EmailAnalysisWorker(text)
    ...
    def _on_analysis_finished(self, analysis):
        ...
        risk = assess_email_risk(analysis, self.language)
        color = LEVEL_COLORS.get(risk["level"], "#e6f1f7")

        level_word = self._t(f"risk_level_{risk['level'].lower()}")
        verdict_label = QLabel(f"{self._t('email_dialog_verdict')}: {level_word}")
        ...
        if not analysis["has_headers"]:
            notice_label = QLabel(self._t("email_dialog_no_headers_notice"))
            notice_label.setWordWrap(True)
            notice_label.setStyleSheet(
                "background-color: #1e3a52; color: #9ac4d8; "
                "padding: 8px; border-radius: 6px; font-style: italic;"
            )
            self.results_layout.addWidget(notice_label)

        if analysis["sender_mismatch"]["mismatch_detected"]:
            sender_box = QGroupBox(self._t("email_dialog_sender_found"))
            ...
            for finding in analysis["sender_mismatch"]["findings"]:
                if finding["type"] == "sender_domain_mismatch":
                    line = QLabel(
                        ...
                        f"{finding['from_address']} "
                        f"(official: {', '.join(finding['official_domains'])})"
                    )
                else:
                    line = QLabel(
                        f"Reply-To: {finding['reply_to_address']} ≠ "
                        f"From: {finding['from_address']}"
                    )
                line.setWordWrap(True)
                line.setStyleSheet("color: #e74c3c;")
                sender_layout.addWidget(line)

            sender_box.setLayout(sender_layout)
            self.results_layout.addWidget(sender_box)

        if analysis["hidden_links"]["hidden_links_detected"]:
            links_box = QGroupBox(self._t("email_dialog_hidden_links_found"))
            links_layout = QVBoxLayout()

            for finding in analysis["hidden_links"]["findings"]:
                line = QLabel(f"\"{finding['displayed_text']}\" → {finding['actual_url']}")
                line.setWordWrap(True)
                line.setStyleSheet("color: #e74c3c;")
                links_layout.addWidget(line)

            links_box.setLayout(links_layout)
            self.results_layout.addWidget(links_box)

        if analysis["has_url"]:
            url_box = QGroupBox(self._t("email_dialog_urls_found"))
        ...
        if analysis["has_suspicious_patterns"]:
            pattern_box = QGroupBox(self._t("email_dialog_patterns_found"))
        ...
        no_findings = (
            ...
            and not analysis["sender_mismatch"]["mismatch_detected"]
            and not analysis["hidden_links"]["hidden_links_detected"]
            and not analysis["time_pressure"]["detected"]
        )

        if no_findings:
            self.results_layout.addWidget(QLabel(self._t("email_dialog_no_findings")))
